...
impl FrameAssembler {
    ...
    pub fn insert(&mut self, drawable_object: &DrawableObject, position: &XY<i32>) {
        for row in 0..drawable_object.get_bitmap().resolution.x {
            for col in 0..drawable_object.get_bitmap().resolution.y {
                ...
                {
                    self.frame.matrix[target_y as usize][target_x as usize] = drawable_object.get_bitmap().matrix[col][row];
                }
            }
        }
    }
        ...
        for row in 0..drawable_object.get_bitmap().resolution.x {
            for col in 0..drawable_object.get_bitmap().resolution.y {
                ...
                if target_x >= 0 && target_y >= 0
                    ...
                    && (target_y as usize) < self.frame.resolution.y
                {
                    self.frame.matrix[target_y as usize][target_x as usize] = drawable_object.get_bitmap_mut().matrix[col][row];
                }
            }
        }
    }
